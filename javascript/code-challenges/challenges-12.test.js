...
const replaceZeros = (string) => {
  let value = /0/;
  while (value.test(string)) {
    string = string.replace(value, 'zero');
  }
  return string;
};
...
const validateWord = (word) => {
  let value = /^[a-zA-Z]{5,10}$/;
  let validate = value.test(word);
  return validate;
};
...
const validateEmail = (email) => {
  let validate = /^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+@[a-zA-Z0-9]+.(com|org|net)$/g;
...
const validatePhoneNumber = (phoneNumber) => {
  let validate = /^(1\s|1|)?((\(\d{3}\))|\d{3})([-|\s])?(\d{3})([-|\s])?(\d{4})$/gm;
