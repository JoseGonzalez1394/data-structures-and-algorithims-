...
const addPurchases = (prev) => {
  let items = prev.reduce((prev, cur) => {
    prev = prev + cur.purchasePrice;
    return prev;
  }, 0);
  return items;
};
...
const countNumberOfElements = (prev) => {
  return prev.reduce((prev,cur,i) => {
    prev = i+1;
    return prev;
  }, 0);
};
...
const returnNames = (arr) => {
  let newArr = arr.reduce((object, key) => {
...
const reversedString = (str) => {
  let arr = str.split('');
  let reversedString = arr.reduce((prev, cur) => {
    return cur + prev;
  }, '');
