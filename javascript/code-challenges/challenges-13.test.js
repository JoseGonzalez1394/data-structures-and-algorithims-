...
const longestString = (Array) => {
  ...
  return Array.indexOf(longestString);
};
...
const firstLetters = (Array) => {
  return Array.map(newArray => newArray[0]);
};
...
const standardizePhoneNumbers = (Array) => {
  return Array.map(number => `${number.substring(1, 4)}${number.substring(6, 9)}${number.substring(10)}`);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const onlyOddChars = (string) => {
  return string.split('').filter((element, index) => index % 2 === 1).join('');
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const allHappy = (Array) => {
  return Array.every(string => string.includes(':)'));
};
