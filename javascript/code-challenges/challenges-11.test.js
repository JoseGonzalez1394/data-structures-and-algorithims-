...
function transformToLis(obj){
  ...
  Object.keys(obj).map((value) => {
    arr.push(`<li>${value}: ${obj[value]}</li>`);
  });
  return arr;
}
...
const count = (integer, input) => {
  return input.flat().filter(value => value === integer).length;
};
...
const totalSum = (input) => {
  return input.flat().reduce((prev, sum) => prev + sum, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  numbersOnly.forEach((v) => {
    newArr.push(v.map(number => Math.pow(2, number)));
  });
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  return data.filter(personObj => personObj.gender.includes('male'|| 'female'))
...
let findShortest = (data) => {
  return data.reduce((shortest, next) => +shortest.height < +next.height ? shortest : next).name;
};
